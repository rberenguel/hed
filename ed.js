    ...
    _processCommand(commandStr) {
        ...
        switch (command[0]) {
            ...
            case 'n':
                ...
                return numPrintOutput.error ? numPrintOutput : { output: numPrintOutput };
            case 's':
                return this._substituteCommand(command, range);
            case 'q':
                return { output: "Type 'w' to save (simulated) and 'q' again to exit.\nOr 'Q' to quit without saving." };
            case 'Q':
                this.buffer = [];
                this.currentLine = 0;
            ...
            case 'H':
                ...
                return { output: `Verbose errors ${this.verboseErrors ? "enabled" : "disabled"}.` };
            case 'P':
...
_parseCommand(commandStr) {
    ...
    _print(start, end) {
        if (this.buffer.length === 0 && (start || end)) return this._error("empty buffer");
