(() => {
  ...
  function getTextFromEditable(element) {
    ...
    clone.querySelectorAll('p, div, pre, h1, h2, h3, h4, h5, h6').forEach(block => {
      block.append('\n');
    });
  ...
  async function createPalette() {
    ...
    const processAndRender = async (command) => {
      ...
      if (!edInstance.inputMode) {
        if (command.startsWith('/') && command.endsWith('H') && command.length > 2) {
    ...
    ['keydown', 'keyup', 'keypress'].forEach(evt => backdrop.addEventListener(evt, (e) => e.stopPropagation()));
